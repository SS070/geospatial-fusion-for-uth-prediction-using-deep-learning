# corrected_unetpp_vae_training.py
import os
import glob
import gc
import pickle
...
for sat, folder in satellite_dirs.items():
    ...
    for file_path in nc_files:
        ...
        try:
            # Expecting filename to contain YYYYMMDD at positions 5:13 as provided
...
# ---------------------------
# Loss: VAE + Temporal + Uncertainty (fixed, non-negative behavior)
# ---------------------------
import torch
import torch.nn as nn
import torch.nn.functional as F
...
class MaskedHybridVAELoss(nn.Module):
    """
    Hybrid masked loss for UNet++ VAE gapfilling:
      - Masked MSE (stability)
      - 1 - MS-SSIM (perceptual sharpness, structure)
      - KL divergence with annealing (latent regularization)
      - Small TV (spatial smoothness at boundaries)

    Also provides make_fused_target(inputs, valid_masks) helper to produce
    the pixel-wise fused target (shape [B,1,H,W]) from per-satellite inputs.
    """
    def __init__(self,
                 alpha=0.6,         # weight for MSE
                 beta=0.4,          # weight for (1 - SSIM)
                 gamma_max=0.01,    # max KL weight
                 tv_weight=5e-3,    # weight for TV
                 kl_warmup_epochs=10):
        super().__init__()
        self.alpha = alpha
        self.beta = beta
    ...
    def make_fused_target(self, inputs, valid_masks):
        """
        inputs: [B,N,1,H,W]
        valid_masks: [B,N,H,W] (0/1 or float)
        returns:
           fused_target: [B,1,H,W]
           fused_mask:   [B,1,H,W] (0/1 float)
        """
        # Ensure float tensors on same device
        inputs = inputs.to(next(self.parameters()).device) if len(list(self.parameters())) else inputs
        device_local = inputs.device
        valid_masks = valid_masks.to(device_local)

        # Mask as float
        m = (valid_masks > 0).float()  # [B,N,H,W]
        m_unsq = m.unsqueeze(2)        # [B,N,1,H,W]

        numerator = (inputs * m_unsq).sum(dim=1)     # [B,1,H,W]
        denom = m_unsq.sum(dim=1).clamp(min=1e-6)    # [B,1,H,W]
        fused = numerator / denom
        fused_mask = (denom > 0).float()
    ...
    def _masked_msssim(self, pred, target, mask):
        # pred, target: [B,1,H,W], mask: [B,1,H,W]
        if mask.ndim == 3:
            mask = mask.unsqueeze(1)
        # If mask covers whole image, just compute full-image MS-SSIM
        mask_bool = bool(mask.all())
        if mask_bool:
            # ms_ssim expects (B,C,H,W) same shapes
            return 1.0 - ms_ssim(pred, target, data_range=1.0, size_average=True)
        else:
            # Apply mask - crude masked MS-SSIM: zero out invalid pixels
            pred_masked = pred * mask
            target_masked = target * mask
            # if masked images are all zeros or identical shapes, ms_ssim might fail; wrap
            try:
                return 1.0 - ms_ssim(pred_masked, target_masked, data_range=1.0, size_average=True)
            except Exception:
                # fallback to 1 - SSIM computed on central crop or to MSE-like surrogate
                # compute channel-wise ssim on numpy where mask ==1 if possible, else fallback to mse-based value
                return 1.0 - ms_ssim(pred_masked, target_masked, data_range=1.0, size_average=True)
    ...
    def forward(self, model_output, target, mask, epoch=0):
        """
        model_output: dict from model with 'prediction' [B,1,H,W], 'mu', 'logvar' optionally
        target: either fused target [B,1,H,W] OR raw inputs [B,N,1,H,W]
        mask: either fused mask [B,1,H,W] OR raw valid_masks [B,N,H,W]
        """
        pred = model_output['prediction']
        mu = model_output.get('mu', None)
        logvar_z = model_output.get('logvar', None)

        # If the user passed raw per-satellite inputs/masks, create fused target and mask
        if target is not None and target.ndim == 5:
            # target is [B,N,1,H,W] -> fuse it
            fused_target, fused_mask = self.make_fused_target(target, mask)
        else:
            fused_target = target
            fused_mask = mask

        # Ensure shapes match: pred [B,1,H,W], fused_target [B,1,H,W], fused_mask [B,1,H,W]
        if fused_target is None:
            raise ValueError("No target provided to loss.")
        if fused_mask.ndim == 3:
            fused_mask = fused_mask.unsqueeze(1)

        # Compute components
        mse_loss = self._masked_mse(pred, fused_target, fused_mask)
        ssim_loss = self._masked_msssim(pred, fused_target, fused_mask)
        tv_loss = self._tv_loss(pred, fused_mask)
        ...
        # Total
        total = self.alpha * mse_loss + self.beta * ssim_loss + kl_loss + self.tv_weight * tv_loss

        metrics = {
            'total': float(total.item()),
            'mse': float(mse_loss.item()),
            'ssim': float(ssim_loss if isinstance(ssim_loss, float) else ssim_loss.item()),
...
criterion = MaskedHybridVAELoss().to(device)

# GradScaler updated to default (works with current torch)
scaler = GradScaler()
...
def train_one_epoch(model, dataloader, optimizer, criterion, device, epoch, scaler):
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        # Update progress bar - use keys present in metrics
...
def train_model():
    ...
    # Save the final model once with both extensions
    print("\nSaving final model...")

    final_model_path_pth = os.path.join(CKPT_DIR, "UNET++_VAE_Optim.pth")
    torch.save({
        'epoch': epoch,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
    ...
    print(f"✓ Model saved as: {final_model_path_pth}")

    final_model_path_h5 = os.path.join(CKPT_DIR, "UNET++_VAE_optim.h5")
    torch.save({
        'epoch': epoch,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
    ...
    print(f"✓ Model saved as: {final_model_path_h5}")
...
def calculate_metrics(y_true, y_pred, data_range=None):
    """
    Calculate MSE, RMSE, MAE, PSNR (manual), ERGAS, SSIM (if safe).
    y_true, y_pred should be 2D arrays (same shape) possibly with NaNs.
    """
    ...
    try:
        if mask.all() and y_true.ndim == 2:
            # structural_similarity expects finite arrays of same shape
...
def evaluate_model(model_eval, dataloader, device, num_samples=100):
    ...
    sample_count = 0
    criterion_eval = MaskedHybridVAELoss().to(device)
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating"):
            ...
            # Create target from inputs (on same device)
            target, target_mask = criterion_eval.make_fused_target(inputs, valid_masks)  # tensors
            targets = target.cpu().numpy()   # [B,1,H,W]
            masks = target_mask.cpu().numpy() # [B,1,H,W]
            ...
            for i in range(batch_size):
                ...
                if valid_pixels.sum() > 0:
                    # Keep masked arrays (invalid pixels are left as zeros but reckoned by mask)
                    pred_masked = pred * mask
...
if __name__ == "__main__":
    try:
        ...
        print("   - UNET++_VAE_optim.pth")
        print("   - UNET++_VAE_optim.h5")
